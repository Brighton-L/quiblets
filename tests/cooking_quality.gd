...
func run()->void:
	...
	check(browser!=null and browser.find_child("RecipeNumber",true,false).text=="#1" and browser.find_child("RecipeName",true,false).text=="Plain Stew" and browser.find_child("RecipeRequires",true,false).text=="No named recipe matched" and browser.find_child("RecipeAttracts",true,false).text==game.recipe_attracts_text(GameData.RECIPES[0]),"The browser opens on stew #1 with its name, requirements, and what it attracts")
